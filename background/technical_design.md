...
截至 2026-07-16，独立游戏代码区 `code/` 的实现状态如下。状态只按各里程碑正式范围计数；为后续阶段预留接口或测试锚点不等于该阶段已经启动。
...
`code/` 已形成可执行的 M0–M3 本地文字运行时：v2 状态、8 点行动制、玩家 DTO 隔离、内容哈希锁定、幂等与乐观锁、SQLite 原子事务、持久化存档、Fake/真实 LLM、调用审计、NPC 记忆、D1–D90 故事时钟、事件/决策、夜间模拟、地图、复盘与终局全文均有自动化验收。当前验收基线为后端 64 项、文字终端 6 项、原剧本工具 85 项；关闭 Fake 降级的真实 `qwen3.6-plus` 已完成 D1–D90 回放、31 个可调用 NPC 机会全量角色矩阵，以及 4 类真实提示词对抗测试。

1. M3 已完成真实供应商 D1–D90 回放签字；后续只需在生产部署前落实第三方服务商 SLA、余额告警、并发限额、数据治理和密钥轮换。
2. 在正式浏览器或服务器部署前补齐 MySQL 生产适配器、Cookie 登录、CSRF、会话安全、迁移与备份；SQLite 和 `X-Account-ID` 只作为本地测试边界。
3. M4 实现研究后台、实验分组、匿名导出、权限与数据治理基线。

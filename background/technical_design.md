...
`code/` 已形成可执行的 M0–M3 本地文字运行时：v2 状态、8 点行动制、玩家 DTO 隔离、内容哈希锁定、幂等与乐观锁、SQLite 原子事务、持久化存档、Fake/真实 LLM、调用审计、NPC 记忆、D1–D90 故事时钟、事件/决策、夜间模拟、地图、复盘与终局全文均有自动化验收。当前验收基线为后端 77 项、文字终端 6 项、原剧本工具 85 项；关闭 Fake 降级的真实 `qwen3.6-plus` 已完成 D1–D90 回放、31 个可调用 NPC 机会全量角色矩阵，以及 4 类真实提示词对抗测试。

...
之后客户端会依次显示出身、决策、NPC、行动、日终、地图、知识和复盘菜单。交谈对象菜单会先显示人物姓名、公开身份、简短介绍、本次接触背景和行动点消耗，再由玩家选择并输入自由对话。普通选择只输入序号；排序题逐项选择优先级；分配题逐项填写额度。只有用户名、隐藏密码、NPC 对话原文和确实需要填写的数值需要键入。与 NPC 交谈会调用当前配置的真实 LLM。

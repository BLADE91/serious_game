...
| M3.6 | 使用固定随机种子，让 Fake、相反方向合法网关和真实 Qwen 分别进入 D1–D90 回放；76 个硬决策、89 夜、台账、选择序列和终局完全一致。 |

## 真实连通性验收

第三方 OpenAI 兼容端点 `https://api.qianzhang-ai.cn/v1/chat/completions` 已在 2026-07-23 完成关闭 Fake 降级的复验。`qwen3.6-plus` 回答通过结构、枚举、知识边界、目标事实和软状态一致性校验，随后完成 D1–D90 回放：76 个硬决策、89 次夜间结转。账本、选择序列和终局均与固定种子基线一致。审计仅有 1 条 `openai_compatible/succeeded` 记录，共 3971 tokens、无重试、无降级。
